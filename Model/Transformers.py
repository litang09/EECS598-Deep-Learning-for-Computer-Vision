...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
        # compute the similarity of query i with all the keys
        scores = torch.zeros(K, dtype=query.dtype, device=query.device)
        ...
        out[i] = torch.mv(value.T, weights)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    out = torch.zeros((N, K, M) ,dtype=query.dtype, device=query.device)

    for n in range(N):
        for k in range(K):
            scores = query[n, k] @ key[n].T / math.sqrt(M) # (M, ) @ (M, K) = (K, )
            weights = torch.softmax(scores, dim=0)
            out[n, k] = weights @ value[n]
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    scores = torch.bmm(query, key.permute(0, 2, 1)) / math.sqrt(M) # shape (N, K, K)
    ...
    if mask is not None:
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        weights = torch.masked_fill(scores, mask, -1e9)
        weights_softmax = torch.softmax(weights, dim=2)
    # Replace "pass" statement with your code
    y = torch.bmm(weights_softmax, value) # (N, K, K) @ (N, K, M) = (N, K, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        query_transformed = self.q.forward(query)
        key_transformed = self.k.forward(key)
        value_transformed = self.v.forward(value)

        y, self.weights_softmax = scaled_dot_product_no_loop_batch(query_transformed, key_transformed, value_transformed, mask)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # use mutiple SingleHeadAttetion
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # parallelly compute all the output of selfattetion
        list_of_head_output = [head(query, key, value, mask) for head in self.heads]
        # concatenate the outputs
        concat = torch.cat(list_of_head_output, dim=-1)
        # use linear layer back to dim_in
...
def get_subsequent_mask(seq):
    ...
    # set elements above the main diagonal True
    subsequent_mask = torch.triu(torch.ones((K, K), dtype=seq.dtype, device=seq.device), diagonal=1).bool()
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        out3 = self.attention_cross(dec_inp, enc_inp, enc_inp)
        out4 = self.dropout(self.norm2(out2 + out3))
        out5 = self.feed_forward(out4)
        y = self.dropout(self.norm3(out5))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    y = torch.tensor([i / K for i in range(K)]).reshape(1, K).unsqueeze(2).expand(1, K, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    # dimension index: (1, M)
    i = torch.arange(M).unsqueeze(0)    # (1, M)

    # denominator term: 10000^(2i/M)
    div_term = 10000 ** torch.floor((2 * (i // 2) / M))  # (1, M)
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        enc_out = self.encoder.forward(q_emb_inp)
        mask = get_subsequent_mask(ans_b[:, :-1])
        dec_out = self.decoder.forward(a_emb_inp, enc_out, mask)
