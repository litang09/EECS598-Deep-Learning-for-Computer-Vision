...
def svm_loss_vectorized(
    ...
    margins = scores - correct_class_scores.view(num_train, 1) + 1
    ...
    #############################################################################
    # Replace "pass" statement with your code
    binarys = (margins > 0).to(dtype=X.dtype)
    row_sum = binarys.sum(dim=1)
    binarys[torch.arange(num_train), y] = -row_sum
...
def sample_batch(
    ...
    idx = torch.randint(0, num_train, size=(batch_size,))
...
def softmax_loss_vectorized(
    ...
    loss = torch.sum(-torch.log(prob[torch.arange(num_train), y]))
    loss = loss / num_train + reg * torch.sum(W * W)

    prob[torch.arange(num_train), y] -= 1.0
