...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden_linear = X.mm(W1) + b1
    hidden = hidden_linear.clamp(min=0)
...
def nn_predict(
    ...
    hidden = torch.relu(hidden)    # apply ReLU

    # Second layer: hidden -> scores for each class
    scores = hidden.mm(W2) + b2    # shape (N, C)

    # Take the index of the maximum score as predicted class label
...
def nn_get_search_params():
    ...
    regularization_strengths = [0.0, 1e-3, 1e-2, 1e-1]
