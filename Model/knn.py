...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            diff = x_train_flatten[i] - x_test_flatten[j]
            dists[i, j] = torch.sum(diff * diff)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_train_flatten = x_train.reshape(num_train, -1)  # (num_train, D)
    x_test_flatten = x_test.reshape(num_test, -1)    # (num_test, D)

    for i in range(num_train):
        diff = x_test_flatten - x_train_flatten[i]
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_norm = (x_train_flatten ** 2).sum(dim=1, keepdim=True)  #(num_train, 1)
    x_test_norm = (x_test_flatten ** 2).sum(dim=1, keepdim=True).transpose(0, 1)  #(1, num_test)

    cross = x_train_flatten @ x_test_flatten.transpose(0, 1)

    dists = x_train_norm + x_test_norm - 2 * cross
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    y_pred = torch.zeros(num_test, dtype=torch.int64)  # (num_test, )
    ##########################################################################
    ...
    # Replace "pass" statement with your code
    for j in range(num_test):
        _, knn_indices = torch.topk(dists[:, j], k, largest=False)

        knn_labels = y_train[knn_indices]
        votes = torch.bincount(knn_labels, minlength=knn_labels.max().item()+1)

        y_pred[j] = votes.argmax()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            ...
            x_train_cv = torch.cat([x_train_folds[j] for j in range(num_folds) if j != i], dim=0)
            y_train_cv = torch.cat([y_train_folds[j] for j in range(num_folds) if j != i], dim=0)

            classifier = KnnClassifier(x_train_cv, y_train_cv)
            accuracy = classifier.check_accuracy(x_val, y_val, k, quiet=True)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accs in k_to_accuracies.items():
        ...
        if (mean_acc > best_acc or (mean_acc == best_acc and k < best_k)):
            best_k = k
            best_acc = mean_acc
    ##########################################################################
    #                           END OF YOUR CODE                             #
