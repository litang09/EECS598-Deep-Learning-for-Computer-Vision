...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        assert C == C_, "input and filter channels must match"
            ...
            for f in range(F):
                for i in range(H_out):
                    for j in range(W_out):
                        ...
                        out[n, f, i, j] = torch.sum(x_region * w[f]) + b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
            ...
            for f in range(F):
                for i in range(H_out):
                    for j in range(W_out):
                        h_start = i * stride
                        w_start = j * stride
                        x_region = x_padded[n, :, h_start:h_start+HH, w_start:w_start+WW]
    ...
    def forward(x, pool_param):
        ...
        for i in range(H_out):
          for j in range(W_out):
            ...
            x_region = x[:, :, h_start:h_end, w_start:w_end]
            out[:, :, i ,j] = torch.amax(x_region, dim=(2, 3))
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for i in range(H_out):
            for j in range(W_out):
              ...
              max_vals = torch.amax(window, dim=(2, 3), keepdim=True)
              mask = (window == max_vals)
              dx[:, :, h_start:h_end, w_start:w_end] += mask * dout[: , :, i, j].view(N, C, 1, 1)
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params["W2"] = torch.normal(0, weight_scale, size=(input_dim_fc, hidden_dim), dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        dh2, dW3, db3 = Linear.backward(ds, cache3)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i, F in enumerate(num_filters):
            ...
            if weight_scale == 'kaiming':
                # Use Kaiming initialization for conv layer
                self.params[f"W{layer_index}"] = kaiming_initializer(
                    Din=C, Dout=F, K=3, relu=True, device=device, dtype=dtype
                )
            else:
                # Default Gaussian initialization
                self.params[f"W{layer_index}"] = torch.normal(
                    0, weight_scale, size=(F, C, 3, 3), dtype=dtype, device=device
                )

            self.params[f"b{layer_index}"] = torch.zeros(F, dtype=dtype, device=device)

            if batchnorm:
                self.params[f"gamma{layer_index}"] = torch.ones(F, dtype=dtype, device=device)
                self.params[f"beta{layer_index}"] = torch.zeros(F, dtype=dtype, device=device)

            # Update spatial size for pooling layers
            if i in max_pools:
                H //= 2
                W //= 2
            C = F
        ...
        # ------------------ Linear Layer ------------------
        layer_index = len(num_filters) + 1
        Din = C * H * W
        Dout = num_classes

        if weight_scale == 'kaiming':
            self.params[f"W{layer_index}"] = kaiming_initializer(
                Din=Din, Dout=Dout, relu=False, device=device, dtype=dtype
            )
        else:
            self.params[f"W{layer_index}"] = torch.normal(
                0, weight_scale, size=(Din, Dout), dtype=dtype, device=device
            )

        self.params[f"b{layer_index}"] = torch.zeros(Dout, dtype=dtype, device=device)
        ################################################################
        #                      END OF YOUR CODE                        #
        ################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            use_bn = self.batchnorm
            gamma = self.params[f"gamma{i}"] if use_bn else None
            beta = self.params[f"beta{i}"] if use_bn else None

            # ------ Case 1/3: Conv + (BN?) + ReLU + Pool ------
            if i-1 in self.max_pools:
                if use_bn:
                    out, cache = Conv_BatchNorm_ReLU_Pool.forward(
                        out, W, b, gamma, beta,
                        conv_param, self.bn_params[i-1], pool_param
                    )
                else:
                    # 没有 BN，则直接 Conv + ReLU + Pool
                    a, conv_cache = FastConv.forward(out, W, b, conv_param)
                    s, relu_cache = ReLU.forward(a)
                    out, pool_cache = FastMaxPool.forward(s, pool_param)
                    cache = (conv_cache, relu_cache, pool_cache)

            # ------ Case 2/4: Conv + (BN?) + ReLU ------
            else:
                if use_bn:
                    out, cache = Conv_BatchNorm_ReLU.forward(
                        out, W, b, gamma, beta,
                        conv_param, self.bn_params[i-1]
                    )
                else:
                    # Conv + ReLU
                    a, conv_cache = FastConv.forward(out, W, b, conv_param)
        ...
        dx, grads[f"W{self.num_layers}"], grads[f"b{self.num_layers}"] = \
            Linear.backward(dscores, caches[-1])
        ...
        for i in reversed(range(1, self.num_layers)):
            ...
            # ---- 情况1：Conv + BN + ReLU + Pool ----
            if i-1 in self.max_pools:
                if use_bn:
                    dx, dW, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dx, cache)
                else:
                    # No BN: Conv + ReLU + Pool
                    conv_cache, relu_cache, pool_cache = cache
                    ds = FastMaxPool.backward(dx, pool_cache)
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    #########################################################
    # Replace "pass" statement with your code
    model = DeepConvNet(input_dims=data_dict['X_train'].shape[1:], num_filters=[48, 128, 192, 192, 128], max_pools=[0, 1, 2, 3], weight_scale="kaiming", dtype=dtype, device=device)
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        weight = torch.normal(0, weight_scale, size=(Din, Dout), device=device, dtype=dtype)
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        fan_in = Din * K * K
        weight_scale = (gain / fan_in) ** 0.5
        weight = torch.normal(0, weight_scale, size=(Dout, Din, K, K), device=device, dtype=dtype)
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            sample_var = x.var(dim=0, correction=0)
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        x, x_hat, mu, var, gamma, beta, eps, mode = cache
        N, D = x.shape
        ...
        if mode == 'train':
            dxhat = dout * gamma
            dvar = torch.sum(dxhat * (x - mu) * (-0.5) * (var + eps) ** (-1.5), dim=0)
            dmu = torch.sum(-dxhat / torch.sqrt(var + eps), dim=0) + \
                dvar * torch.mean(-2. * (x - mu), dim=0)
            dx = dxhat / torch.sqrt(var + eps) + dvar * 2 * (x - mu) / N + dmu / N
        else:
    ...
    def backward_alt(dout, cache):
        ...
        # Simplified dx formula (fits on one line)
        dx = (1. / N) * gamma / torch.sqrt(var + eps) * (
            N * dout - torch.sum(dout, dim=0)
            - x_hat * torch.sum(dout * x_hat, dim=0)
        )
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = dout.shape
        # reshape -> vanilla BN backward -> reshape back
        dx, dgamma, dbeta = BatchNorm.backward(
            dout.permute(0,2,3,1).reshape(-1, C),
            cache
        )
