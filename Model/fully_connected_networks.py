...
class ReLU(object):
    ...
    def forward(x):
        ...
        ###################################################
        # Replace "pass" statement with your code
        out = torch.maximum(x, torch.tensor(0.0, device = x.device, dtype = x.dtype))
        ###################################################
        #                 END OF YOUR CODE                #
        ###################################################
        cache = x
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        self.params['W1'] = torch.normal(0, weight_scale, size=(input_dim, hidden_dim), dtype=dtype, device=device)
        self.params['b1'] = torch.zeros(hidden_dim, dtype=dtype, device=device)
        self.params['W2'] = torch.normal(0, weight_scale, size=(hidden_dim, num_classes), dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        hidden, cache1 = Linear_ReLU.forward(X, W1, b1)
        scores, cache2 = Linear.forward(hidden, W2, b2)
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            self.params[f'W{i+1}'] = torch.normal(0, weight_scale, size=(self.dims[i], self.dims[i+1]), dtype=dtype, device=device)
            self.params[f'b{i+1}'] = torch.zeros(self.dims[i+1], dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, L):
            ...
            cache.append(ci)
            if self.use_dropout:
              h, dropout_ci = Dropout.forward(h, self.dropout_param)
              dropout_cache.append(dropout_ci)
        scores, cL = Linear.forward(h, self.params[f'W{L}'], self.params[f'b{L}'])
        cache.append(cL)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        for i in range(L):
            ...
            loss += self.reg * 0.5 * torch.sum(W*W)
        dout, dW, db = Linear.backward(ds, cache[L-1])
        grads[f'W{L}'] = dW + self.reg * self.params[f'W{L}']
        grads[f'b{L}'] = db
        for i in range(L-1, 0, -1):
            if self.use_dropout:
                dout = Dropout.backward(dout, dropout_cache[i-1])
            dout, dW, db = Linear_ReLU.backward(dout, cache[i-1])
