...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    h_raw = x @ Wx + prev_h @ Wh + b
    next_h = torch.tanh(h_raw)

    cache = (x, Wx, prev_h, Wh, b, h_raw, next_h)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x, Wx, prev_h, Wh, b, h_raw, next_h = cache
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0
    for t in range(T):
        # Get input for current timestep: (N, D)
        x_t = x[:, t, :]
...
def rnn_backward(dh, cache):
    ...
    # Initialize gradient from next timestep
    dnext_h = torch.zeros(N, H, device=dh.device, dtype=dh.dtype)
    
    # Backward pass through time (from last timestep to first)
    for t in range(T-1, -1, -1):
        # Total gradient for this timestep = upstream gradient + gradient from next timestep
        dnext_h_total = dh[:, t, :] + dnext_h
        
        # Compute gradients for this timestep
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dnext_h_total, cache[t])
    ...
    # The final dprev_h after processing all timesteps is dh0
    dh0 = dnext_h
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.encoder = ImageEncoder(pretrained=image_encoder_pretrained, verbose=False)
        
        if cell_type == "attn":
            # AttentionLSTM will use CNN feature map directly, so projection
            # layer is optional; but if you want, you can still project for h0
            self.feature_proj = nn.Conv2d(input_dim, hidden_dim, kernel_size=1)
        else:
            # Flatten and project for vanilla RNN/LSTM
            self.feature_proj = nn.Sequential(
                nn.Flatten(),  # (N, C*H*W)
                nn.Linear(input_dim*4*4, hidden_dim)
            )
    ...
    def forward(self, images, captions):
        ...
        # Replace "pass" statement with your code
        # 1. Encode images
        features = self.encoder(images)  # (N, C, H', W') or (N, D)

        # 2. Initial hidden state or CNN feature map
        if self.cell_type == "attn":
            # AttentionLSTM uses raw CNN features A (N, H, H', W') directly
            h0 = None
            A = self.feature_proj(features)  # (N, H, 4, 4)
        else:
            # Flatten + project features to initial hidden state h0
            h0 = self.feature_proj(features)
            A = None

        # 3. Embed words
        x = self.embed(captions_in)  # (N, T, W)

        # 4. RNN/LSTM/AttentionLSTM forward
        if self.cell_type == "attn":
            h = self.rnn(x, A)         # AttentionLSTM expects x and A
        else:
            h = self.rnn(x, h0)        # Vanilla RNN/LSTM expects x and h0

        # 5. Temporal affine projection to vocab scores
        scores = self.output_proj(h)    # (N, T, V)

        # 6. Temporal softmax loss, ignore <NULL>
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == "attn":
            # 投影 CNN 输出到 hidden_dim
            A = self.feature_proj(features)      # (N, H, 4, 4)
            h = A.mean(dim=(2,3))                # 初始 hidden state
            c = h.clone()                        # 初始 cell state
            attn_weights_all = images.new(N, max_length, 4, 4).fill_(0).float()
        else:
            h = self.feature_proj(features)      # RNN/LSTM 初始 hidden state
            c = torch.zeros_like(h) if self.cell_type == "lstm" else None
            A = None

        word = torch.full((N,), fill_value=self._start, dtype=torch.long, device=images.device)

        for t in range(max_length):
            x = self.embed(word)  # (N, W)
            ...
            if self.cell_type == "rnn":
                ...
                h, c, attn_weights = self.rnn.step_forward(x, h, c, A)
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        a_i, a_f, a_o, a_g = torch.chunk(a, 4, dim=1)
...
def dot_product_attention(prev_h, A):
    ...
    attn_scores = torch.matmul(prev_h.unsqueeze(1), A_flat).squeeze(1)    # (N, 16)
    attn_weights_flat = torch.nn.functional.softmax(attn_scores / math.sqrt(H), dim=1)    # (N, 16)
    attn = torch.matmul(A_flat, attn_weights_flat.unsqueeze(2)).squeeze(2)    # (N, H)
    attn_weights = attn_weights_flat.reshape(N, 4, 4)    # (N, 4, 4)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Affine transform with 3 sources: x, prev_h, attn
        activation = (
            x @ self.Wx +
            prev_h @ self.Wh +
            attn @ self.Wattn +
        ...
        # Split gates
        i, f, o, g = torch.chunk(activation, 4, dim=1)
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        for t in range(T):
            ...
            # Compute attention using previous h
            attn, attn_w = dot_product_attention(prev_h, A)  # (N,H), (N,4,4)
        ...
        hn = torch.stack(h_list, dim=1)  # (N, T, H)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        return hn
